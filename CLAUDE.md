...
3. **Review cap:** up to five adversarial review rounds, each answered by one revision — fix what matters most first; a small
   edge case earns a note, not machinery. If critical findings persist past round two or three, step back as the owner would
   and weigh simplifying, another approach, splitting the work smaller, or reaching the goal another way; a re-scope restarts
   as a fresh brief and intake row, telling the owner. Trust-critical work with an unresolved critical finding stays BLOCKED: escalate, never ship on a spent cap.
