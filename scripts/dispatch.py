...
def exclusive_lock(lock: Path):
    """NON-REENTRANT: code inside must not acquire this same lock—in particular write_state().
    Keep using the unlocked writer (atomic_write).
    """
...
def worker_prompt_text(att: Path, lc: dict, n: int) -> str:
    ...
    preamble = (
        ...
        "your work.\n"
        "When you finish, the working tree must hold only your change: delete any virtualenvs, "
        "caches, or scratch files you created — a symlink, FIFO, socket, or device node left "
        "anywhere in the tree (a .venv/bin/python, for example) fails the whole attempt.\n"
