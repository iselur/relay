...
if [ ! -x "$PY" ] || ! "$PY" -c 'import yaml, jsonschema' 2>/dev/null; then
  echo "SKIP dispatch_base_pin.sh: .venv/pyyaml/jsonschema absent (dispatcher self-test needs the dispatcher venv; CI installs it)"
  exit 77
fi
...
def run_preflight(base):
    # A schema-valid, instance-bound, spec-bound, in-scope approval (B1) so the base-pin check — not
    # approval shape — is what this test exercises.
